...
export default function ImpersonationBanner({
    ...
    const handleEnd = async () => {
        setEnding(true);
        try {
            const res = await api.post<{ accessToken: string }>('/admin/impersonate/end');
            // Yeni token'ı cookie/localStorage'e yaz
    ...
    return (
        <div className="sticky top-0 z-[60] flex items-center justify-center gap-3 bg-amber-500 px-4 py-2 text-sm font-medium text-white shadow-md">
            ...
            <span>
                <strong>{impersonating.name}</strong> olarak görüntülüyorsunuz ({impersonating.role})
            </span>
            <button
                type="button"
                onClick={handleEnd}
