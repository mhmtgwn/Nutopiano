...
export default function AdminUserDetailPage() {
  ...
  } = useDraftState<UserFormData>(initialFormData);
  ...
  const updateProfileMutation = useMutation({
    ...
    onError: (err: unknown) =>
      toast.error(resolveApiErrorMessage(err, "Güncelleme başarısız.")),
  });

  const roleMutation = useMutation({
    mutationFn: async (payload: { role: UserRole; reason?: string }) => {
      ...
      return api.patch(`/users/${userId}/role`, { role: payload.role });
    },
    onSuccess: async () => {
  ...
  const toggleActiveMutation = useMutation({
    mutationFn: async () =>
      api.patch(`/users/${userId}/active`, { isActive: !user?.isActive }),
    onSuccess: async () => {
      toast.success("Durum güncellendi.");
      await queryClient.invalidateQueries({ queryKey: ["admin-user", userId] });
    },
    onError: (err: unknown) =>
      toast.error(resolveApiErrorMessage(err, "İşlem başarısız.")),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err: unknown) =>
      toast.error(resolveApiErrorMessage(err, "Silme başarısız.")),
  });
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-semibold text-[var(--primary-800)]">
                {user.name}
              </h1>
              <StatusBadge
                variant={roleVariant[toSystemRole(user.role)] ?? "neutral"}
              >
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="rounded-xl border border-[var(--neutral-200)] bg-white p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-base font-semibold text-[var(--primary-800)]">
                Profil Bilgileri
              </h2>
              {!editMode ? (
                <button
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                <label className="mb-1 block text-[11px] font-medium text-[var(--neutral-500)]">
                  AD SOYAD
                </label>
                {editMode ? (
                  <input
              ...
              <div>
                ...
                {requiresRoleOverride ? (
                  <input
                    ...
                    className="mt-2 w-full rounded-lg border border-[var(--neutral-200)] px-3 py-2 text-sm outline-none focus:border-[var(--primary-400)]"
                  />
                ) : null}
              </div>
            </div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-4 border-t border-[var(--neutral-100)] pt-4">
              <div>
                <p className="text-[11px] font-medium text-[var(--neutral-500)]">
                  KAYIT TARİHİ
                </p>
                <p className="text-sm text-[var(--primary-800)]">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-[var(--neutral-200)] bg-white p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-sm font-semibold text-[var(--primary-800)]">
          ...
          {/* Quick Actions */}
          <div className="rounded-xl border border-[var(--neutral-200)] bg-white p-5">
            <h3 className="text-sm font-semibold text-[var(--primary-800)] mb-3">
              Hızlı İşlemler
            </h3>
