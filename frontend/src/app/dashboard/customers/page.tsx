...
export default function SellerCustomersPage() {
  ...
  const canEditPolicy = role === 'SELLER' || role === 'ADMIN' || role === 'SUPER_ADMIN';
  ...
  const [draftCreditLimitCents, setDraftCreditLimitCents] = useState('');
  const [draftCreditPolicy, setDraftCreditPolicy] = useState<CreditPolicy>('WARN');
  ...
  useEffect(() => {
    ...
    if (!selectedCustomerId || !customers.some((row) => row.id === selectedCustomerId)) {
  ...
  const ledgerQuery = useQuery<SellerCustomerLedgerResponse>({
    queryKey: ['seller-customer-ledger', { selectedCustomerId, ledgerPage, ledgerPageSize }],
    enabled: typeof selectedCustomerId === 'number' && selectedCustomerId > 0,
    queryFn: async () => {
      const res = await api.get<SellerCustomerLedgerResponse>(
        `/seller/customers/${selectedCustomerId}/ledger`,
        {
  ...
  useEffect(() => {
    if (!ledgerQuery.data?.customer) return;
    setDraftCreditLimitCents(
      typeof ledgerQuery.data.customer.creditLimitCents === 'number'
        ? String(ledgerQuery.data.customer.creditLimitCents)
        : '',
    );
    setDraftCreditPolicy(ledgerQuery.data.customer.creditBlockPolicy);
  }, [ledgerQuery.data?.customer?.id]);
  ...
  const updateCreditPolicyMutation = useMutation({
    mutationFn: async () => {
      ...
      await api.patch(`/seller/customers/${selectedCustomerId}/credit-policy`, payload);
    },
    onSuccess: async () => {
      toast.success('Kredi politikasi guncellendi.');
      await queryClient.invalidateQueries({ queryKey: ['seller-customers'] });
      await queryClient.invalidateQueries({ queryKey: ['seller-customer-ledger'] });
    },
    onError: (error: unknown) => {
      toast.error(resolveApiErrorMessage(error, 'Kredi politikasi guncellenemedi.'));
    },
  });
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-[var(--radius-xl)] border border-[var(--neutral-200)] bg-white px-6 py-6">
        ...
        <h1 className="mt-2 text-2xl font-serif text-[var(--primary-800)]">Musteriler</h1>
        <p className="mt-2 text-sm text-[var(--neutral-600)]">
          Veresiye bakiyeleri ve musteri hareketlerini seller scope icinde yonetin.
        </p>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_1.2fr]">
        <section className="rounded-[var(--radius-xl)] border border-[var(--neutral-200)] bg-white px-6 py-6">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <h2 className="text-xl font-serif text-[var(--primary-800)]">Musteri listesi</h2>
          ...
          {customersQuery.isLoading && <Spinner fullscreen label="Musteriler yukleniyor..." />}
          ...
          {customersQuery.isError && !customersQuery.isLoading && (
            <div className="mt-4 rounded-[var(--radius-lg)] border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
              {resolveApiErrorMessage(customersQuery.error, 'Musteriler yuklenemedi.')}
            </div>
          )}
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-1 text-xs text-[var(--neutral-600)]">{row.phone}</p>
                      </div>
          ...
          <div className="mt-5 flex items-center justify-between gap-3">
            ...
            <button
              type="button"
              onClick={() => setPage((prev) => Math.min(customersPaging.totalPages, prev + 1))}
            ...
            <>
              <div className="flex flex-wrap items-end justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-[var(--neutral-600)]">{selectedCustomer.phone}</p>
                </div>
                <div className="rounded-full border border-[var(--neutral-200)] bg-white px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-[var(--primary-800)]">
                  Acik borc: {formatPrice(selectedCustomer.outstandingDebtCents)}
                </div>
              </div>

              {ledgerQuery.isLoading && <Spinner fullscreen label="Ledger yukleniyor..." />}

              {ledgerQuery.isError && !ledgerQuery.isLoading && (
                <div className="mt-4 rounded-[var(--radius-lg)] border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                  {resolveApiErrorMessage(ledgerQuery.error, 'Ledger verisi yuklenemedi.')}
                </div>
              )}

              {!ledgerQuery.isLoading && !ledgerQuery.isError && ledgerQuery.data && (
                <>
                  <div className="mt-4 grid gap-3 md:grid-cols-3">
                    <div className="rounded-[var(--radius-lg)] border border-[var(--neutral-200)] bg-[var(--neutral-50)] px-4 py-3">
                      ...
                      <p className="mt-1 text-sm font-semibold text-[var(--primary-800)]">
                        {formatPrice(ledgerQuery.data.summary.totalDebitCents)}
                      </p>
                    </div>
                    <div className="rounded-[var(--radius-lg)] border border-[var(--neutral-200)] bg-[var(--neutral-50)] px-4 py-3">
                      <p className="text-[10px] uppercase tracking-[0.15em] text-[var(--neutral-500)]">
                      ...
                      <p className="mt-1 text-sm font-semibold text-[var(--primary-800)]">
                        {formatPrice(ledgerQuery.data.summary.totalCreditCents)}
                      </p>
                    </div>
                    <div className="rounded-[var(--radius-lg)] border border-[var(--neutral-200)] bg-[var(--neutral-50)] px-4 py-3">
                      <p className="text-[10px] uppercase tracking-[0.15em] text-[var(--neutral-500)]">
                      ...
                      <p className="mt-1 text-sm font-semibold text-[var(--primary-800)]">
                        {formatPrice(ledgerQuery.data.summary.outstandingDebtCents)}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 rounded-[var(--radius-lg)] border border-[var(--neutral-200)] bg-[var(--neutral-50)] px-4 py-4">
                    <div className="grid gap-3 md:grid-cols-[1fr_220px_150px]">
                      <label className="text-xs font-semibold text-[var(--neutral-600)]">
                        ...
                        <input
                          ...
                          onChange={(e) => setDraftCreditLimitCents(e.target.value)}
                      ...
                      <label className="text-xs font-semibold text-[var(--neutral-600)]">
                        ...
                        <select
                          ...
                          onChange={(e) => setDraftCreditPolicy(e.target.value as CreditPolicy)}
                          disabled={!canEditPolicy}
                          className="mt-1 h-10 w-full rounded-[var(--radius-lg)] border border-[var(--neutral-200)] bg-white px-3 text-sm outline-none disabled:cursor-not-allowed disabled:bg-[var(--neutral-100)]"
                        >
                      ...
                      <div className="flex items-end">
                        <button
                          ...
                          disabled={!canEditPolicy || updateCreditPolicyMutation.isPending}
                    ...
                    {!canEditPolicy ? (
                      <p className="mt-2 text-xs text-[var(--neutral-600)]">
                        Bu alanda degisiklik yetkisi sadece SELLER/ADMIN rollerindedir.
                      </p>
                    ) : null}
                  </div>
                      ...
                      <tbody>
                        {ledgerQuery.data.data.map((row) => (
                          <tr key={row.id} className="border-b border-[var(--neutral-100)]">
                            ...
                            <td className="py-3 pr-3 text-xs text-[var(--neutral-700)]">{row.sourceType}</td>
                  ...
                  <div className="mt-5 flex items-center justify-between gap-3">
                    ...
                    <button
                      type="button"
                      onClick={() => setLedgerPage((prev) => Math.min(ledgerPaging.totalPages, prev + 1))}
